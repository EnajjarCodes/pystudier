...
const QuizPanel = ({ userName, userId, chatContext, onQuizComplete, onIncorrectQuestions, initialTopic, initialSubject, sessionMode, reviewMode, reviewQuestionCount, dynamicQuestionCount }: QuizPanelProps) => {
  ...
  const [setup, setSetup] = useState<QuizSetup>({
    ...
    questionCount: reviewMode ? (reviewQuestionCount || 5) : (dynamicQuestionCount || 10),
    checkAnswers: true,
  ...
  // Auto-generate quiz when in session mode with initialTopic
  useEffect(() => {
    if (initialTopic && !autoGenerated) {
  ...
  const generateQuiz = async () => {
    const topic = setup.topic || initialTopic || "";
    if (!topic.trim()) return;
    setPhase("loading");
    setError("");

    try {
      const questionTypes = setup.questionTypes.length > 0 ? setup.questionTypes : ["multiple_choice", "true_false", "written"];
      ...
      // Filter out matching questions if they made it through
      const filteredQuestions = (data.questions || []).filter((q: QuizQuestion) => q.type !== "matching");
      if (filteredQuestions.length === 0) throw new Error("No valid questions generated");
  ...
  const finishQuiz = async () => {
    ...
    try {
      await supabase.from("study_progress" as any).insert({
        ...
        topic: setup.topic || initialTopic,
      });
    } catch {}

    onQuizComplete?.(score, questions.length, setup.topic || initialTopic || "");

    // Check unchecked answers
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4 sm:space-y-6 min-h-0">
        ...
        <div className="flex gap-2 items-start">
          ...
          <div className="rounded-2xl rounded-bl-md bg-card shadow-card px-3 py-2">
            <p className="text-xs sm:text-sm text-foreground font-body">Here's what to fix.</p>
          </div>
        </div>
        ...
        {/* Only show New Quiz if NOT in session mode */}
        {!sessionMode && (
